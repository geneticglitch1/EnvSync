...
pub async fn run(key: String, message: Option<String>, api_url: &str) -> Result<()> {
    ...
    let mut env_map: BTreeMap<String, String> =
        match vc.get_latest_snapshot(&project.project_id).await? {
            ...
            None => {
                anyhow::bail!("No remote snapshot found; nothing to unset.");
            }
        };

    if !env_map.contains_key(&key) {
        anyhow::bail!("Key '{key}' not found in the vault.");
    }
    env_map.remove(&key);
    ...
    let msg = message.unwrap_or_else(|| format!("Unset {key}"));
    let snap = vc
        .push_snapshot(
            &project.project_id,
            PushSnapshotRequest {
    ...
    println!("Unset {key} → pushed v{}", snap.version);
    Ok(())
}
